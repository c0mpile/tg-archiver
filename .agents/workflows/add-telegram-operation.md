...
3. The method must be wrapped in the flood-wait retry helper already defined
   in `src/telegram/`. A call to any grammers method that bypasses this helper
...
5. If the operation uploads a file, use `client.upload_file()` with chunked
   streaming. Do not buffer the entire file into a `Vec<u8>`.
6. If the operation iterates messages, use `client.iter_messages()` with a
   chunk size of 100. Process and persist state after each chunk — do not
   accumulate all results before processing.
