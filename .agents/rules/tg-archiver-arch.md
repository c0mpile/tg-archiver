...
- Scanning chunk size is 100 messages via `get_messages_by_id`.
- Service message skip heuristic: `text().trim().is_empty() && media().is_none()`.
- `random_id` generation: must be `base_micros + i as i64` via enumerate — never duplicated within a batch.
- Delay: 500ms inter-chunk delay.
- Cursor update: updated to `current_end` after each chunk.
- Topic auto-creation: `auto_create_topic: bool` on State triggers `create_topic()` before the run starts and is reset to `false` after.
...
2. State tests use `unsafe { std::env::set_var }` causing races under parallel test execution. Run `cargo test -- --test-threads=1` until fixed.