...
3. **TUI State Transition (`src/app/mod.rs`)**:
   ...
   - We will spawn a Tokio task that:
     1. Checks if `new_id == state.source_channel_id`. If so, do not reload state (no-op reload to prevent wiping). Just proceed to `GroupSelect`.
     2. Otherwise, atomically saves the *current* channel's state if `state.source_channel_id.is_some()`.
     3. Calls `State::load_for_channel(new_id).await`. If the file does not exist, it naturally returns `State::default()`.
     4. Sets the new `source_channel_id` / title on the newly loaded state.
     5. Dispatches a new `AppEvent::ChannelStateLoaded(State)` to the main loop.
...
- Add safety checks to `State::save()` to return `Ok(())` if `source_channel_id` is missing, and log `eprintln!("State::save called with no source_channel_id, skipping save");`.
